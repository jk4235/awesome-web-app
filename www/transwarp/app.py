...
class UTC(datetime.tzinfo):
    def __init__(self, utc):
        ...
        if mt:
            ...
            if minus:
                h, m = (-h), (-m)
            self._utccoffset = datetime.timedelta(hours=h, minutes=m)
            self._tzname = 'UTC%s' % utc
        else:
            raise ValueError('bad utc time zone')

    def utcoffset(self, dt):
        return self._utccoffset
...
# def favicon_handler():
#     return static_file_handler('favcoin.ico')
...
def _default_error_handler(e, start_response, is_debug):
    if isinstance(e, HttpError):
        ...
        return ('<html><body><h1>%s</h></body></html>' % e.status)
...
class WSGIApplication(object):
    ...
    def _check_not_running(self):
        ...
        def add_module(self, mod):
            ...
            for name in dir(m):
                fn = getattr(m, name)
                if callable(fn) and hasattr(fn, '__web_route__') and hasattr(fn, '__web_methos__'):
                    self.add_url(fn)
