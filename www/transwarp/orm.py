...
class Model(dict):
    ...
    def count_by(cls, where, *args):
        return db.select_int('select count(%s) from %s %s' % (cls.__primary_key__.name, cls.__tabel__, where), *args)

    def update(self):
        self.pre_update and self.pre_update()
        L = []
        args = []
        for k, v in self.__mappings__.iteritem():
            if v.updatable:
                if hasattr(self, k):
                    arg = getattr(self, k)
                else:
